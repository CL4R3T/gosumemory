		...
		if memory.DynamicAddresses.IsReady == true {
			...
			if memory.GameplayData.GameMode == 3 {
				if maniaSR > 0 {
					...
					memory.GameplayData.PP.Pp = int32(calculateManiaPP(maniaSR, maniaHitObjects, memory.GameplayData.Hits.HGeki, memory.GameplayData.Hits.H300, memory.GameplayData.Hits.HKatu, memory.GameplayData.Hits.H100, memory.GameplayData.Hits.H50, memory.GameplayData.Hits.H0))
				}
			}
		}
